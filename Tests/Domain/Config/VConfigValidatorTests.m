...
#import "../../../VrInteractiveTracking/Domain/Config/VConfigValidator.h"
#import "../../../VrInteractiveTracking/Common/XMLParser.h"

@interface VConfigValidatorTests : XCTestCase

@end

@implementation VConfigValidatorTests
...
- (void)testInit {
    ...
    NSString *filePath = [self getFilePath:@"vrTrackingVConfig_validateNomal"];
    NSString *identity = @"v";
    NSString *date = @"";
    BOOL isRemoteFile = NO;
    
    __weak XCTestExpectation *expectation = [self expectationWithDescription:@"vrTrackingVConfig_validateNomal"];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
        ...
        [xmlParser parseXML:filePath identity:identity date:date isRemoteFile:isRemoteFile callback:^(NSMutableArray *configFileArray, BOOL isRemoteFile) {
            ...
            for (id <Config> config in configFileArray) {
                ...
                @try {
                    // 正常ファイルか判断
                    VConfigValidator *validator = [[VConfigValidator alloc] initWithValidationHandler:[ConfigValidationHandler new] config:config];
...
- (void)testValidateAbomalNotTag {
    ...
    NSString *filePath = [self getFilePath:@"vrTrackingVConfig_validateNotTag"];
    NSString *identity = @"v";
    NSString *date = @"";
    BOOL isRemoteFile = NO;
    
    __weak XCTestExpectation *expectation = [self expectationWithDescription:@"vrTrackingVConfig_validateNotTag"];
...
- (void)testValidateAbomalNotValue {
    ...
    NSString *filePath = [self getFilePath:@"vrTrackingVConfig_validateNotValue"];
    NSString *identity = @"v";
    NSString *date = @"";
    BOOL isRemoteFile = NO;
    
    __weak XCTestExpectation *expectation = [self expectationWithDescription:@"vrTrackingVConfig_validateNotValue"];
...
- (void)testInstansiateIsNull0 {
    ...
    VConfigValidator *validator = [VConfigValidator new];
...
- (void)testInstansiateIsNull1 {
    ...
    VConfigValidator *validator = [[VConfigValidator alloc] initWithValidationHandler:nil config:nil];
...
- (void)testInstansiateIsNull2 {
    ...
    VConfigValidator *validator = [[VConfigValidator alloc] initWithValidationHandler:[ConfigValidationHandler new] config:nil];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
        ...
        [xmlParser parseXML:filePath identity:identity date:date isRemoteFile:isRemoteFile callback:^(NSMutableArray *configFileArray, BOOL isRemoteFile) {
            ...
            for (id <Config> config in configFileArray) {
                ...
                @try {
                    // 正常ファイルか判断
                    VConfigValidator *validator = [[VConfigValidator alloc] initWithValidationHandler:nil config:config];
                    config.isNormal = ![[validator handler] errors];
                    NSLog(@"%@", config.getElements);
                    NSLog(@"%@", [[validator handler] toString]);
